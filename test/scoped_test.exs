defmodule EctoRanked.ScopedTest do
  ...
  describe "insertions" do
    ...
    test "allow the same rank for different scopes" do
      ...
      refute Repo.get(Model, scope1_model1.id).my_rank == Repo.get(Model, scope1_model2.id).my_rank
      assert Repo.get(Model, scope1_model1.id).my_rank == Repo.get(Model, scope2_model1.id).my_rank
    end
    ...
    test "treats a missing scope as its own scope" do
      ...
      assert Repo.get(Model, scope1_model1.id).my_rank == Repo.get(Model, noscope_model1.id).my_rank
    end
  end
end
